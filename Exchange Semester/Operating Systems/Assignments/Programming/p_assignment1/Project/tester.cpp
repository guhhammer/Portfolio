...
// This function is a secondary function, used to check if a number is on an array.
...
void tester::secondTest(void)
{
	...
	for (int k = 0; k < numberOfNodes; k++) { randomNodes[k] = -1; }

	// Defining random values to be chosen furtherly.
	for (int i = 0; i < numberOfNodes; i++) {

		int x = rand() % numberOfNodes + 1;

		while (isInArray(randomNodes, numberOfNodes, x)) { x = rand() % numberOfNodes + 1; }
	...
	for (int k = 0; k < numberOfSteps; k++) {
		...
		if (k % 100000 == 0) { std::cout << "\n" << std::to_string((k / 10000)) << "%  (" << totalTime << " ms) \n"; }
