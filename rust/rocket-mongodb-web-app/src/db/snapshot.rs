...
async fn get_database_state(name: &str) -> Result<DatabaseState, Box<dyn Error + Send + Sync>> {
    let client = Client::with_options(ClientOptions::parse(db_config::get_db_uri()).await?)?;
    let database = client.database(&db_config::get_db_name());
    ...
    loop {
        ...
        if name == db_config::COLLECTION_NAMES[0] {
            ...
            for n in db_config::COLLECTION_NAMES {
                ...
                if let Some(last) =  v_files.clone().into_iter().rfind(|x| x.contains(n)) {
