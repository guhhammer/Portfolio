...
create table tbEstudante(
    ...
    foreign key (Instituicao) references tbInstituicao(chave),
    foreign key (Curso) references tbCurson(chave)
);

create table tbAcademico(
	CPF char(11),
    Area_Estudo int,
    Instituicao int,  -- especificar instituição tabela separada.
    primary key(CPF),
    foreign key (CPF) references tbIndividuo(CPF),
    foreign key (Area_Estudo) references tbArea(chave),
    foreign key (Instituicao) references tbInstituicao(chave)
);
...
create table tbNaoAcademico(
    ...
    primary key(CPF),
    foreign key (CPF) references tbIndividuo(CPF),
    foreign key (Ocupacao) references tbOcupacao (chave)
);
...
insert into tbCurson values(0, "Ciencia da Computacao");
insert into tbCurson values(1, "Engenharia da Computacao");
insert into tbCurson values(2, "Jogos Digitais");
insert into tbCurson values(3, "Medicina");

insert into tbInstituicao values(0, "PUCPR");
insert into tbInstituicao values(1, "UFPR");
insert into tbInstituicao values(2, "UTFPR");
insert into tbInstituicao values(3, "USP");

insert into tbArea values(0, "Matematica");
insert into tbArea values(1, "Fisica");

insert into tbOcupacao values(0,"Freelancer");
insert into tbOcupacao values(1,"Empresario");

insert into tbIndividuo values("13829381928","Jose Silva");
insert into tbEstudante values("13829381928", 1,0,0);

insert into tbIndividuo values("29381928391","Amanda Souza");
insert into tbEstudante values("29381928391", 1, 0,0);

insert into tbIndividuo values("92839182902","Joao Henrique");
insert into tbEstudante values("92839182902", 1, 1,2);

insert into tbIndividuo values("88291829182","Gabriela Farias");
insert into tbEstudante values("88291829182", 1, 2,0);

insert into tbIndividuo values("88884444222","Gabriel Frias");
insert into tbEstudante values("88884444222", 2, 3,0);

insert into tbIndividuo values("82739182738","Carlos Borges");
insert into tbAcademico values("82739182738", 0, 0);

insert into tbIndividuo values("82889910928","Fernanda Silveira");
insert into tbAcademico values("82889910928", 0, 3);

insert into tbIndividuo values("82888991029","Vinicius Nunes");
insert into tbAcademico values("82888991029", 1,1);

insert into tbIndividuo values("29102938192","Maria Ferreira");
insert into tbNaoAcademico values("29102938192", 0);

insert into tbIndividuo values("82918299809","Rodrigo Pereira");
insert into tbNaoAcademico values("82918299809", 0);

insert into tbIndividuo values("82938173928","Eduardo Lopes");
insert into tbNaoAcademico values("82938173928", 1);

insert into tbIndividuo values("82938173000","João Silva");
insert into tbNaoAcademico values("82938173000",1);
...
insert into tbInscricao values("82938173000",1,"Ouvinte","2018-02-12",40.0);
...
select c.nome, count(*)
from tbCurson c, tbEstudante es, tbIndividuo i, tbInscricao sc, tbEvento e
where es.CPF = i.CPF and i.CPF = sc.CPF_Individuo and sc.UID_Evento = e.UID and es.Curso = c.chave
group by c.nome
order by count(*) desc
limit 1;
...
-- Últimos 6 exercícios.  Atividade do dia 30/05/2019
...
3. Qual palestra foi mais atendida por persoas não-academica?

*/
select p.Tema, count(p.tema)
from tbIndividuo i, tbNaoAcademico na, tbPalestra p, tbPresenca pr
where  i.CPF = na.CPF and i.CPF = pr.CPF_Individuo and pr.UID_Palestra = p.UID
order by(count(p.tema)) desc 
limit 1;

-- Retorna a palestra com o maior o número de não acadêmicos que a assitiram.
...
10 - Quais forão as palestras efetuadas por professores?

*/
select i.nome, p.Tema 
from tbIndividuo i, tbAcademico a, tbPAlestra p, tbPresenca pr
where i.CPF = a.CPF and i.CPF = pr.CPF_Individuo and pr.UID_Palestra = p.UID
order by(i.nome);
...
/*
 ...
 7- Quais são os nomes dos  ouvintes não-acadêmicos?

*/
select i.nome
from tbIndividuo i, tbNaoAcademico na, tbInscricao ins
where i.CPF = na.CPF and ins.CPF_Individuo = i.CPF and ins.Papel_Individuo = "Ouvinte"
order by(i.nome);
...
Grupo: -Bruno Henrique BarbosaAlves,Guilherme Henrique  Fenner Hey,Pedro Henrique da Silva Churata

 3) Qual o tema da palestra em que Amanda Souza participou?
 
*/
select i.nome, p.Tema
from tbIndividuo i, tbPresenca pr, tbPalestra p
where i.nome = "Amanda Souza" and i.CPF = pr.CPF_Individuo and pr.UID_Palestra = p.UID;



/*
Grupo: Matheus Teixeira, Luan Magalhães, Lucas Yanaga, João Pedro de Lima
8) Quantas palestras ocorreram em 2018?
*/
select count(*)
from tbPalestra p 
where year(p.Data_Palestra) = 2018;


/*
Grupo: Matheus Teixeira, Luan Magalhães, Lucas Yanaga, João Pedro de Lima

3. Quantas pessoas estavam presentes na palestra sobre o AZURE – Microsoft?
*/
select count(*)
from tbPalestra p, tbPresenca pr, tbIndividuo i
where p.UID = pr.UID_Palestra and pr.CPF_Individuo = i.CPF and p.Tema = "AZURE - Microsoft";
