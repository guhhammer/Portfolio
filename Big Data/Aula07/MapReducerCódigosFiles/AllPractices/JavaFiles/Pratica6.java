...
public class Pratica6 {
    ...
    public static class MapperImplementacao6 extends Mapper<Object, Text, Text, IntWritable>{

        public void map(Object chave, Text valor, Context context) throws IOException, InterruptedException {

            String[] b = valor.toString().split(";");

            if(b[0].trim().contains("Brazil") && b[1].trim().contains("2016")){
    ...
    public static class ReducerImplementacao6 extends Reducer<Text, IntWritable, Text, IntWritable> {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        String arquivoSaida = System.getProperty("user.home") + "/Desktop/" + Pratica6.class.getSimpleName();
        if(args.length == 2){
            arquivoEntrada = args[0];
            arquivoSaida = args[1];
        }
        Configuration conf = new Configuration();
        Job job = Job.getInstance(conf, String.valueOf(Pratica6.class.getSimpleName()));

        job.setJarByClass(Pratica6.class);
        job.setMapperClass(MapperImplementacao6.class);
        job.setReducerClass(ReducerImplementacao6.class);
