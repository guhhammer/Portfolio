...
public class Pratica15 {
    ...
    public static class MapperImplementacao15 extends Mapper<Object, Text, Text, IntWritable>{

        public void map(Object chave, Text valor, Context context) throws IOException, InterruptedException {

            String[] b = valor.toString().split(";");
            context.write(new Text("<"+b[0]+", "+b[4]+", "+b[1]+">"), new IntWritable(1));

        }

    }

    public static class ReducerImplementacao15 extends Reducer<Text, IntWritable, Text, IntWritable> {
        public void reduce(Text chave, Iterable<IntWritable> valores, Context context) throws IOException, InterruptedException {

            int qt = 0;
            for(IntWritable v : valores){ qt +=v.get(); }
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        String arquivoSaida = System.getProperty("user.home") + "/Desktop/" + Pratica15.class.getSimpleName();
        if(args.length == 2){
            arquivoEntrada = args[0];
            arquivoSaida = args[1];
        }
        Configuration conf = new Configuration();
        Job job = Job.getInstance(conf, String.valueOf(Pratica15.class.getSimpleName()));

        job.setJarByClass(Pratica15.class);
        job.setMapperClass(MapperImplementacao15.class);
        job.setReducerClass(ReducerImplementacao15.class);
