...
Ex.2 => 	12n² = O(n)
			...
			... Contradiction: c would have to be at least k * n with k > 12, so it is not a constant 
...
# 4 ** (n+3) = O( 4 ** n) |- Valid.
# Note: c = 128 was used so the difference shows in the chart; any c above 64 is already valid.
...
# 4 ** (n+3) = O( 4 ** n) |- Contradiction: n survives the simplification, so c is not a constant.
...
# 6n² + 8n = Θ(n²) |- Contradiction: c2 does not exceed f(n).
...
# 12n³ + 10n² ≠ Θ(n²) |- Valid. C2 varia em n.
...
# 4n² + 16n + 2 = Θ(n³) |- Contradiction. Omega varia em x, c1*g(n) > f(n).
