"""Train a small CNN to tell faces with and without a mask, from webcam captures (class demo).

While the camera runs, press a to capture a face with mask, b without mask, q to stop; the captures are split 80/20, the CNN is trained for 10 epochs and the best model is saved to model-faces.model. Needs faces.xml (OpenCV Haar cascade).
"""
import cv2
