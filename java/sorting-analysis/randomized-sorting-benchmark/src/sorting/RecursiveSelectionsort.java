...
public class RecursiveSelectionsort {
	...
	// selectionsortRandom(...) -> C1 + ... + C13 = 9 + 4n = O( n )
	//
	private static int selectionsortRandom(int[] arr, int k){

		if(arr.length == 1){  return arr[0]; }								// C1 <- 1
		else{

			int j = partition(arr, randomGenerator.nextInt(arr.length));		// C2 <- n
			...
			if(j == k){														// C6 <- 1
			    ...
			    R = null;													// C9 <- 1
				return selectionsortRandom(L, k);							// C10 <- n
			    ...
			    L = null;													// C11 <- 1
				k = k - j - 1;												// C12 <- 1
				return selectionsortRandom(R, k);							// C13 <- n
	...
	// selectionsortControl(...) -> C1 + ... + C4 = 2 + n + n² = O( n² )
	//
	private static int[] selectionsortControl(int[] arr, int counter, int[] Q){

		if(counter == arr.length){ return Q; }							// C1 <- 1
		else{

			Q = append(Q, selectionsortRandom(arr, counter));			// C2 <- n
			selectionsortControl(arr, counter + 1, Q);		// C3 <- n * n
	...
	//	selectionsortRandomWrapper(...) -> C1 = n² = O( n² )
	//
	public static void selectionsortRandomWrapper(int[] arr){

		selectionsortControl(arr, 0, new int[]{});		// C1 <- n²
