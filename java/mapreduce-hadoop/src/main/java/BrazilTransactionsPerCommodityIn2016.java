...
/**
 * Practice 6: number of Brazilian transactions per commodity in 2016.
 *
 ...
 */
public class BrazilTransactionsPerCommodityIn2016 {

    public static class TradeMapper extends Mapper<Object, Text, Text, IntWritable> {

        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            String[] cols = value.toString().split(";");
            if (cols[0].trim().contains("Brazil") && cols[1].trim().contains("2016")) {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        String outputPath = System.getProperty("user.home") + "/Desktop/" + BrazilTransactionsPerCommodityIn2016.class.getSimpleName();
        if (args.length == 2) {
            inputPath = args[0];
            outputPath = args[1];
        }

        Job job = Job.getInstance(new Configuration(), BrazilTransactionsPerCommodityIn2016.class.getSimpleName());
        job.setJarByClass(BrazilTransactionsPerCommodityIn2016.class);
