...
/**
 * Practice 15: number of transactions per country, flow and year.
 *
 ...
 */
public class TransactionsPerCountryFlowAndYear {

    public static class TradeMapper extends Mapper<Object, Text, Text, IntWritable> {

        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            String[] cols = value.toString().split(";");
            context.write(new Text("<" + cols[0] + ", " + cols[4] + ", " + cols[1] + ">"), new IntWritable(1));
        }
    }
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        String outputPath = System.getProperty("user.home") + "/Desktop/" + TransactionsPerCountryFlowAndYear.class.getSimpleName();
        if (args.length == 2) {
            inputPath = args[0];
            outputPath = args[1];
        }

        Job job = Job.getInstance(new Configuration(), TransactionsPerCountryFlowAndYear.class.getSimpleName());
        job.setJarByClass(TransactionsPerCountryFlowAndYear.class);
