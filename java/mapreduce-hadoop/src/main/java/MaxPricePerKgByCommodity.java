...
/**
 * Practice 14: highest price per kg (trade_usd / weight_kg) per commodity.
 *
 ...
 */
public class MaxPricePerKgByCommodity {
    ...
    public static class TradeMapper extends Mapper<Object, Text, Text, DoubleWritable> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            try {
                ...
                context.write(new Text(cols[3]), new DoubleWritable(pricePerKg));
            } catch (Exception e) {
    ...
    public static class TradeReducer extends Reducer<Text, DoubleWritable, Text, DoubleWritable> {
        ...
        public void reduce(Text key, Iterable<DoubleWritable> values, Context context) throws IOException, InterruptedException {
            ...
            for (DoubleWritable v : values) {
                double m = Math.max(v.get(), max);
                max = (m == Double.POSITIVE_INFINITY) ? max : m;   // weight 0 -> skip
            }
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        String outputPath = System.getProperty("user.home") + "/Desktop/" + MaxPricePerKgByCommodity.class.getSimpleName();
        if (args.length == 2) {
            inputPath = args[0];
            outputPath = args[1];
        }

        Job job = Job.getInstance(new Configuration(), MaxPricePerKgByCommodity.class.getSimpleName());
        job.setJarByClass(MaxPricePerKgByCommodity.class);
