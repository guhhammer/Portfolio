...
public class FILA { 
    ...
    private final Semaphore dado[];
    private static int primeiro = 0, ultimo = 0, MAX, tamanho = 0; 
    
    public FILA(int MAX){ FILA.MAX = MAX; this.dado = new Semaphore[FILA.MAX]; }
    
    public int max(){return FILA.MAX;}
    
    protected boolean vazia(){ return (tamanho == 0);}
    
    protected boolean cheia(){ return (tamanho == MAX-1);}
    
    protected Semaphore primeiro(){ return dado[FILA.primeiro];}
    
    protected Semaphore ultimo(){ return dado[FILA.ultimo];}
