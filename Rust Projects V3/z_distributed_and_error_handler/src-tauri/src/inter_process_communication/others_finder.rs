...
pub const CLEANUP_REFRESH_RATE: u64 = 5;

pub const BUFFER_SIZE: [u8; 1024] = [0u8; 1024];
...
fn cleanup_peers(peer_list: PeerList) {
    let timeout = Duration::from_secs(TIMEOUT); // adjust as needed.

    thread::spawn(move || {
        loop {
            thread::sleep(Duration::from_secs(CLEANUP_REFRESH_RATE));
...
fn start_listener(socket: UdpSocket, peer_list: &PeerList, display: bool) {
    ...
    thread::spawn(move || {
        let mut buf = BUFFER_SIZE; // buffer = max message size. (CHANGE ACCORDINGLY)
...
use crate::error_catalog::udp::BROADCAST_ERROR_SEND;
...
fn broadcast_presence(socket: UdpSocket, addr: SocketAddr, instance_name: String) {
    loop {
        ...
        if let Err(e) = socket.send_to(msg.as_bytes(), addr) {
            ...
            println!("{}", 
            BROADCAST_ERROR_SEND.clone().with_details(&e.to_string()));
