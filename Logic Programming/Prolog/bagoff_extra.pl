...
ex1(Cidades) :- bagof(C,Y^cidade(C,Y,argentina),Cidades).


ex2(Cidades,Estado) :- bagof(C,Cidades^cidade(C,Estado,brasil),Cidades).


ex3(Cidades, Estado) :- bagof(C,Cidades^cidade(C,Estado,_),Cidades).



ex4(Estado,Pais) :- setof(Estado,X^cidade(X,Estado,Pais),Estado).


ex5(Estado, Pais) :- bagof(Estado,X^cidade(X,Estado,Pais),Estado).
