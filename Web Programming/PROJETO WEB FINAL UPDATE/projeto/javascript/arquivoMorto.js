...
$(document).ready(function(){
	...
	$.ajax({
		...
		async: false,
		data: {
			caminho: ("../xml/emails/"+(emailVAR)+"/arquivoMorto/"),
		},
	...
	while((emailsJSONVAR).length > counter){
		...
		var link = ("../xml/emails/"+emailVAR+"/arquivoMorto/"+emailsJSONVAR[index]);
		var referencia = "../html/arquivoMorto.html";
		...
		string += "<td><p id=\"PARA\">Para: "+destinatario+"</p></td>";
	...
	$("#mensagens #msgBox #btExcluir").each(function(){
		...
		$(this).click(function(){
			...
			dest = dest.replace(emailVAR+"/arquivoMorto/", "");
			...
			$.ajax({
				...
				async: false,
				data: {
					emailAtual: emailVAR, arcName: dest, actualPaste: "arquivoMorto",
				},
