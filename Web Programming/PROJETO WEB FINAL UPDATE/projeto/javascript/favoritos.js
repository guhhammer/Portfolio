...
$(document).ready(function(){
	...
	$.ajax({
		...
		async: false,
		data: {
			caminho: ("../xml/emails/"+(emailVAR)+"/favoritos/"),
		},
	...
	while((emailsJSONVAR).length > counter){
		...
		var link = ("../xml/emails/"+emailVAR+"/favoritos/"+emailsJSONVAR[index]);
		var referencia = "../html/favoritos.html";
		...
		string += "<td><p id=\"DE\">DE: "+emailCc+"</p></td>";
	...
	var notRED = false;
 	...
 	$("#mensagens #msgBox #btExcluir").each(function(){
		...
		$(this).click(function(){
			...
			dest = dest.replace(emailVAR+"/favoritos/", "");
			...
			$.ajax({
				...
				async: false,
				data: {
					emailAtual: emailVAR, arcName: dest, actualPaste: "favoritos",
				},
	...
	$("#mensagens #msgBox").click(function(){
		...
		if(!notRED){
			...
			$(this).click(function(){
				...
				linkS = linkS.replace((emailVAR+"/favoritos/"), "");
		        ...
		        $.ajax({
		        	...
		        	async: false,
		        	data: {
		        		dest: emailVAR, cc: deQuem, ass: assS, msg: msgS, fromScreen: "../html/favoritos.html", link: linkS,
		        	},
