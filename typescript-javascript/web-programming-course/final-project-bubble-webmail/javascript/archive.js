...
$(document).ready(function(){
	...
	$.ajax({
		...
		async: false,
		data: {
			caminho: ("../xml/emails/"+(emailVAR)+"/archive/"),
		},
	...
	while((emailsJSONVAR).length > counter){
		...
		var link = ("../xml/emails/"+emailVAR+"/archive/"+emailsJSONVAR[index]);
		var referencia = "../html/archive.html";
	...
	$("#messages #msgBox #btDelete").each(function(){
		...
		$(this).click(function(){
			...
			dest = dest.replace(emailVAR+"/archive/", "");
			...
			$.ajax({
				...
				async: false,
				data: {
					emailAtual: emailVAR, arcName: dest, actualPaste: "archive",
				},
			...
			window.open("../html/archive.html", "_self");
	...
	$("#messages #msgBox").each(function(){
		...
		$(this).click(function(){
			...
			if(!notRED){
		        ...
		        window.open("../html/composeMessage2.html", "_self");
