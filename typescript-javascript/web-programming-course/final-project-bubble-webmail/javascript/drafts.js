...
$(document).ready(function(){
	...
	$.ajax({
		...
		async: false,
		data: {
			caminho: ("../xml/emails/"+(emailVAR)+"/drafts/"),
		},
	...
	while((emailsJSONVAR).length > counter){
		...
		var link = ("../xml/emails/"+emailVAR+"/drafts/"+emailsJSONVAR[index]);
		var referencia = "../html/drafts.html";
	...
	$("#messages #msgBox #btDelete").each(function(){
		...
		$(this).click(function(){
			...
			dest = dest.replace(emailVAR+"/drafts/", "");
			...
			$.ajax({
				...
				async: false,
				data: {
					emailAtual: emailVAR, arcName: dest, actualPaste: "drafts",
				},
