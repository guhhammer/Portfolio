...
$(document).ready(function(){
	...
	$.ajax({
		...
		async: false,
		data: {
			caminho: ("../xml/emails/"+(emailVAR)+"/favorites/"),
		},
	...
	while((emailsJSONVAR).length > counter){
		...
		var link = ("../xml/emails/"+emailVAR+"/favorites/"+emailsJSONVAR[index]);
		var referencia = "../html/favorites.html";
	...
	var notRED = false;
 	...
 	$("#messages #msgBox #btDelete").each(function(){
		...
		$(this).click(function(){
			...
			dest = dest.replace(emailVAR+"/favorites/", "");
			...
			$.ajax({
				...
				async: false,
				data: {
					emailAtual: emailVAR, arcName: dest, actualPaste: "favorites",
				},
			...
			window.open("../html/favorites.html", "_self");
	...
	$("#messages #msgBox").click(function(){
		...
		if(!notRED){
			...
			$(this).click(function(){
				...
				linkS = linkS.replace((emailVAR+"/favorites/"), "");
		        ...
		        $.ajax({
		        	...
		        	async: false,
		        	data: {
		        		dest: emailVAR, cc: deQuem, ass: assS, msg: msgS, fromScreen: "../html/favorites.html", link: linkS,
		        	},
